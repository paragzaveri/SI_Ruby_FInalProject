module NationalHockeyLeague
	def nhl_simulate_season(league_1,league_2)
		while @nhl_game_count <= 1229
			...
			if ((league_1["#{first_team}"][0] + league_1["#{first_team}"][1] + league_1["#{first_team}"][2]) < 82) && ((league_2["#{second_team}"][0] + league_2["#{second_team}"][1] + league_2["#{second_team}"][2]) < 82)
				...
				nhl_simulate_season(league_1,league_2)
			end
		end	
	end
	def nhl_generate_conference_game(league)
		keys = league.keys
 		first_team = keys[rand(15)]
		second_team = keys[rand(15)]

		if ((league["#{first_team}"][0] + league["#{first_team}"][1] + league["#{first_team}"][2]) < 82) && ((league["#{second_team}"][0] + league["#{second_team}"][1] + league["#{second_team}"][2]) < 82)
